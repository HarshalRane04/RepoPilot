...
- Applied-patch provider eval harness through `repopilot_evals.ProviderAppliedPatchEvalRunner` and `make provider-applied-patch-eval`; local runs copy fixtures to temporary workspaces, apply model-generated unified diffs, run benchmark-declared validation commands, and report patch-quality evidence without mutating fixtures.
