...
- Executed security scanner snapshot through `scripts/security_scanner_snapshot.py`, `make security-scanner-snapshot`, and the CI scanner artifact job. The target runs Semgrep, resolves Python declarations into a pinned CPython 3.12/Linux set before `pip-audit`, runs `npm audit`, requires CodeQL evidence from the clean current revision, records tool versions/results, and binds the report to a release source fingerprint under `Docs/release-artifacts/` or uploaded workflow artifacts.
