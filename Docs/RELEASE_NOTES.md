...
- Release workflow evidence job in `.github/workflows/release.yml`; manual and tag-triggered release runs now upload deterministic local eval, source-boundary manifest, credential-smoke, and deployment-validation artifacts before API/web/sandbox image builds. Dry run `27059971285` passed and its downloaded artifact hashes are archived under `Docs/release-artifacts/release-workflow-dry-run.*`.
