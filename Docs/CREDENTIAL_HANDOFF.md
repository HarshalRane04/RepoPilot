...
| Setting | Purpose |
|---|---|
| `SEMGREP_ENABLED=true` | Enables command-backed Semgrep in the owned CI/release evidence environment. `make security-scanner-snapshot` provisions it on demand; it is not exposed to runtime agents. |
| `DEPENDENCY_AUDIT_ENABLED=true` | Enables release-boundary dependency evidence. Python declarations are resolved with `uv` for CPython 3.12/Linux and audited as a pinned transitive set with `pip-audit`; the npm production lockfile is audited separately. |
