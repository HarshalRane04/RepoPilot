...
| P0 | Release gates allow blocked/failing evidence in some local artifact commands. | `--allow-blocked`, `--allow-warnings`, and `--allow-failures` are useful for local placeholder builds, but a release tag or image-publishing run needs strict behavior. | Strict release targets now exist in `Makefile`, and tag-triggered plus manually publishing release workflow runs now omit blocked/failed-gate allowances. The next proof step is to run them after credentials and Docker runtime are available. |
...
| PRD-03 | First-run setup checklist | P1 | Dashboard Settings shows ordered blockers and "next action" for GitHub App, OAuth, model, scanners, and OTLP; docs now include `make init-local-env`, source-build `make up`, and released-image `make ghcr-start-local` for fresh-clone-safe GHCR startup. |
...
| PRD-07 | Artifact retention policy | P1 | Configurable local artifact retention settings and dry-run/delete cleanup planner now exist; remaining hardening is scheduled cleanup wiring plus object-store/signed-download deployment support. |
...
|---|---|---|
| `services/*` directories | Most service directories are README-only planned extraction boundaries; `services/sandbox_runner/Dockerfile` is active because local builds, CI, GHCR, and sandbox validation use it. | Keep only if README and architecture docs call README-only directories future extraction points, while sandbox_runner is labeled as active image plus future service boundary. Do not imply README-only directories are deployable services. |
