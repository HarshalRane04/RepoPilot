...
**June 11 sub-agent audit results now tracked:** independent low-effort GPT-5.5 audits identified the following remaining production-readiness gaps: first-run local env initialization has been addressed for local demos through `make init-local-env`, and the GHCR release workflow plus `docker-compose.ghcr.yml` released-image install path are now implemented locally; remaining proof requires an actual tag/manual publish run, package visibility check, and fresh-host GHCR smoke. Live GitHub write/model/eval proof remains credential-blocked; UI labels overstate some local/manual flows (`Start`, PR readiness, CI revision planning, manual CI log analyzer); scaffold-only service directories can confuse open-source users; and repository indexing with live embeddings remains an external code-transfer decision that needs explicit repo opt-in before production claims.
