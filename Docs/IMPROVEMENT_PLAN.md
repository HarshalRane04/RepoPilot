...
| RP-12-03 | Release workflows | API/web/sandbox image builds, tags, release notes, changelog. | Partially complete: release notes exist for the local v1.0 candidate, and the release workflow now has a manual/tag-triggered release-evidence job that uploads deterministic eval, source-boundary, and deployment-validation artifacts before image builds; manual dry run `27059971285` passed with evidence upload plus API/web/sandbox image builds, and downloaded artifact hashes are archived under `Docs/release-artifacts/release-workflow-dry-run.*`. Final changelog remains pending. |
