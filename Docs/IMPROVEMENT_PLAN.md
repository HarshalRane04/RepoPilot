...
**Current status:** Partially implemented locally. The deployment guide, demo script, release checklist, case study, release notes, core static screenshots, local visual-flow GIFs, scanner posture snapshot, credential smoke summary, and `scripts/deployment_validate.py` now exist in release-usable form and describe or verify the local stack, released-image GHCR path, single-VM path, managed Postgres/Redis option, secret handling, backups, rollback, local demo flow, credentialed GitHub smoke flow, Compose topology, env placeholders, release artifacts, and release evidence gates. `docker-compose.ghcr.yml` defines the prebuilt-image install path for API, worker, beat, web, Postgres, Redis, and sandbox image pulls without source bind mounts or dev commands. `make ghcr-config`, `make ghcr-pull`, `make ghcr-up`, and `make ghcr-migrate` expose that path to installers. `make release-gifs` writes `Docs/release-artifacts/release-gifs.md`/`.json`, `make credential-smoke` writes `Docs/release-artifacts/credential-smoke-summary.md`/`.json`, `make security-scanner-snapshot` writes `Docs/release-artifacts/security-scanner-snapshot.md`/`.json`, `make deployment-validate` writes `Docs/release-artifacts/deployment-validation.md`, and `make deployment-smoke` writes `Docs/release-artifacts/deployment-runtime-smoke.md`/`.json`; both deployment checks currently pass for the local stack. `.github/workflows/release.yml` now supports manual dry runs, optional manual image publishing, and tag-triggered release evidence generation; it uploads deterministic local eval, source-boundary manifest, credential-smoke, and deployment-validation artifacts, then builds API/web/sandbox images with GHCR metadata, pushes them on tag releases or explicitly published manual runs, and uploads image digest evidence. Remaining work is proof beyond local validation: actual GHCR publish/package-visibility smoke, production-like cloud/VM deployment smoke, live GitHub smoke evidence, enabled external-scanner proof, and provider-backed eval evidence are still pending.
