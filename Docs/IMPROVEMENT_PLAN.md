...
| Artifact storage | `apps/api/app/services/artifacts.py`, `ArtifactRecord`, migration `0004_artifact_records`, `GeneratedPatch.diff_artifact`, and `ValidationResult.log_artifact` now externalize patch diffs, validation logs, and large tool outputs behind `local://artifacts/...` references with SHA-256 and byte-size metadata. Local artifact files are written owner-only where supported, and `ArtifactStore.plan_retention` can dry-run or delete expired files under `REPOPILOT_ARTIFACT_STORE_ROOT` without following symlink/path escapes. | The object-store-ready contract exists locally with configurable local retention; cloud object storage, signed download URLs, and automated artifact cleanup scheduling remain future deployment work. |
...
| Local runtime platform | Docker Compose now runs API, web, worker, beat, Postgres, and Redis together. Web dev caches are isolated through named volumes for `node_modules` and `.next`; API/worker/beat share `agent_workspaces` for sandbox cleanup and `agent_artifacts` for artifact-backed evidence. `make start-local`/`make bootstrap` initializes local env values, starts the source-build stack, applies migrations, builds the sandbox image, and prints the local URLs; `make ghcr-start-local` now initializes local env values before pulling/starting released images; `make deployment-smoke` writes local runtime API/web smoke evidence under `Docs/release-artifacts/`. | First-run source-build and GHCR install friction is lower; actual GHCR publish/package visibility, credentialed GitHub/model proof, and production cloud/VM deployment proof are still pending. |
