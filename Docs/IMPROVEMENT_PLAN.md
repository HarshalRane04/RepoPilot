...
5. Dry-run the release workflow through `workflow_dispatch` and inspect the uploaded release-evidence artifact.
...
| RP-12-03 | Release workflows | API/web/sandbox image builds, tags, release notes, changelog. | Partially complete: release notes exist for the local v1.0 candidate, and the release workflow now has a manual/tag-triggered release-evidence job that uploads deterministic eval, source-boundary, and deployment-validation artifacts before image builds; workflow dry run and final changelog remain pending. |
