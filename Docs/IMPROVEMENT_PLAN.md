...
| Scanner posture evidence | `.github/workflows/codeql.yml`, `scripts/security_scanner_snapshot.py`, `make security-scanner-snapshot`, and the CI `scanner-posture` job now capture external scanner enablement, dependency manifest coverage, CodeQL workflow presence, tool availability/version evidence, and explicit disabled/blocked reasons under local release artifacts or uploaded CI artifacts. | R6 release evidence is stronger locally and in CI for Semgrep/dependency-audit posture; CodeQL still needs code-scanning/alert evidence before production security claims are complete. |
