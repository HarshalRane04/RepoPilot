...
**June 11 sub-agent audit results now tracked:** independent low-effort GPT-5.5 audits identified the following remaining production-readiness gaps: first-run local env initialization has been addressed for local demos through `make init-local-env`, but Docker Compose remains dev-oriented (`uvicorn --reload`, `npm run dev`, bind mounts) without a production compose/profile; release workflow builds images but does not publish GHCR images; live GitHub write/model/eval proof remains credential-blocked; UI labels overstate some local/manual flows (`Start`, PR readiness, CI revision planning, manual CI log analyzer); scaffold-only service directories can confuse open-source users; and repository indexing with live embeddings remains an external code-transfer decision that needs explicit repo opt-in before production claims.
...
**Current status:** Partially implemented locally. The deployment guide, demo script, release checklist, case study, release notes, core static screenshots, local visual-flow GIFs, scanner posture snapshot, credential smoke summary, and `scripts/deployment_validate.py` now exist in release-usable form and describe or verify the local stack, single-VM path, managed Postgres/Redis option, secret handling, backups, rollback, local demo flow, credentialed GitHub smoke flow, Compose topology, env placeholders, release artifacts, and release evidence gates. `make release-gifs` writes `Docs/release-artifacts/release-gifs.md`/`.json`, `make credential-smoke` writes `Docs/release-artifacts/credential-smoke-summary.md`/`.json`, `make security-scanner-snapshot` writes `Docs/release-artifacts/security-scanner-snapshot.md`/`.json`, `make deployment-validate` writes `Docs/release-artifacts/deployment-validation.md`, and `make deployment-smoke` writes `Docs/release-artifacts/deployment-runtime-smoke.md`/`.json`; both deployment checks currently pass for the local stack. `.github/workflows/release.yml` now supports manual dry runs and tag-triggered release evidence generation, uploads deterministic local eval, source-boundary manifest, credential-smoke, and deployment-validation artifacts, and gates API/web/sandbox image builds on that evidence job. Manual release dry run `27059971285` passed with `release-evidence` and `build-images` jobs green; downloaded artifact contents and hashes are archived in `Docs/release-artifacts/release-workflow-dry-run.md` and `.json`. Remaining work is proof beyond local validation: production-like cloud/VM deployment smoke, live GitHub smoke evidence, enabled external-scanner proof, and provider-backed eval evidence are still pending.
