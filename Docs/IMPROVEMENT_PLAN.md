...
- Eval reports are fixture-backed with per-task outcomes, executable fixture repositories, observed plan/retrieval/patch-quality scoring, applied-patch validation harnesses, human edit distance, and provider-comparison ranking, but credentialed model-generated applied-patch results and CI-backed eval runs remain future hardening.
...
June 12 sidecar audits added three release-readiness lanes: install/onboarding, security/privacy, and practical dead-feature/agent-harness risk. Closed immediately: stale CodeQL recommendation YAML now matches the checked-in CodeQL v4 workflow, the local-vs-real PR contract is explicit across API/UI, `make start-local`/`make bootstrap` provides a one-command source-build startup path, first-run docs now lead with that path, protected Runbook curls include local dev auth headers, the repository-index prompt no longer hardcodes a developer machine path, repository/issue/run/PR/eval operator read surfaces now require authentication with object checks on repo/issue/PR detail and triage mutation, saved GitHub/model config now clears stale verification markers while provider model verification fails closed when the selected model is not confirmed by the provider, implementation-agent `workspace.apply_patch` calls skip the immediate diff payload because final patch capture already runs `workspace.diff`, and webhook persistence now stores minimized/redacted processing envelopes with raw payload SHA-256 evidence instead of full GitHub payloads. June 17 hardening added README first-run prerequisites/failure modes, minimized free-form `/repopilot` command audit metadata to presence/hash/length, corrected operator-console labels that overstated PR/security/notification/readiness states, changed scanner release proof so a local CodeQL executable is not enough, and dispatched CodeQL after setting `CODEQL_ENABLED=true`; GitHub ran analysis but upload failed because repository code scanning is not enabled. Still open from the audits: stronger write-smoke evidence binding, retry-context freshness, broader large-workspace copy efficiency, and successful CodeQL proof after GitHub code scanning is enabled.
...
| Retrieval | `apps/api/app/services/repo_indexer.py` routes chunk and query embeddings through `ModelGateway.embed`, normalizes vectors to the existing pgvector dimension, stores embedding provider/model/dimensions on chunks, writes dedicated `repository_indexes` metadata through migration `0005_repository_index_metadata`, marks stale indexes by embedding and chunker version, chunks Python/TypeScript/JavaScript/Go/Rust/Java/Markdown around symbols/headings where practical, classifies source/test/doc/config chunks, rejects symlink escapes, skips `.secrets`, secret-like files, generated dependency trees, generated bundle files, and private-key suffixes before chunk storage, and keeps live embedding source transfer disabled unless `EMBEDDING_SOURCE_TRANSFER_ENABLED=true`. | Context search is now gateway-mediated, cited, safer, privacy-gated, and metadata-backed; live provider retrieval-quality benchmarks remain pending after explicit source-transfer opt-in. |
| Implementation | `apps/api/app/services/implementation_agent.py` creates an isolated workspace through `workspace.create_run_copy`, asks the model for bounded workspace write tool calls, avoids redundant apply-patch diff snapshots, captures final diff/patch hash, normalizes pytest commands into the sandbox's active Python runtime, and validates through the sandbox validation tool. | Executor-mediated codegen path is in place; live model quality, retry-context freshness, broader large-workspace copy efficiency, and full issue-to-patch benchmark proof are still pending. |
| Tool boundary | `apps/api/app/services/tools/registry.py` exposes model-facing tools and enforces approved-plan/workspace gates, typed block reasons, redacted persisted tool output, approved-plan hash matching, approved write paths before mutation, secret-like file exclusion from run-workspace copies, and secret-like file exclusion from repo read/list/grep/tree tools. | Stronger foundation after June 11 hardening; next risk areas are model-facing context retention minimization, content DLP on ordinary files, and batch-read efficiency. |
| Artifact storage | `apps/api/app/services/artifacts.py`, `ArtifactRecord`, migration `0004_artifact_records`, `GeneratedPatch.diff_artifact`, and `ValidationResult.log_artifact` now externalize patch diffs, validation logs, and large tool outputs behind `local://artifacts/...` references with SHA-256 and byte-size metadata. Local artifact files are written owner-only where supported, and `repopilot.artifacts.retention_cleanup` schedules `ArtifactStore.plan_retention` through Celery Beat with dry-run enabled by default. | The object-store-ready contract exists locally with scheduled dry-run/deletion retention for local files; cloud object storage and signed download URLs remain future deployment work. |
| Authorization | `apps/api/app/services/authorization.py` provides role and object-access helpers for repositories, issues, plans, runs, PRs, and security findings; state-changing routes and broad operator list/detail routes now require authentication, with repo/issue/PR object-access checks on known object endpoints and triage mutation. | Stronger single-tenant authorization baseline; credentialed GitHub collaborator evidence and deeper list filtering remain future hardening. |
...
| Credential readiness | `.env.example`, Docker Compose, `IntegrationReadinessService`, `/settings/github/app`, `/settings/github/app/verify`, `/settings/readiness`, and the dashboard settings screen now distinguish missing credentials, present-but-unverified credentials, verified read-only GitHub mode, unverified write mode, verified write mode markers, mock model mode, unverified live model mode, and verified live model mode. Runtime secret tombstones allow saved settings to clear stale verification markers, GitHub App config saves clear prior app/write-smoke verification, model config saves clear prior model verification, and provider model verification only persists verified state when the selected model is confirmed. | R2 is stronger locally; live verification still requires user-provided GitHub App credentials, installation ID, provider keys, and a demo repository, and write-smoke proof should be upgraded from timestamp-only to structured evidence. |
| Scanner posture evidence | `.github/workflows/codeql.yml`, `scripts/security_scanner_snapshot.py`, `make security-scanner-snapshot`, and the CI `scanner-posture` job now capture external scanner enablement, dependency manifest coverage, CodeQL workflow presence, explicit CodeQL run/SARIF/alert evidence status, tool availability/version evidence, and disabled/blocked reasons under local release artifacts or uploaded CI artifacts. A dispatched CodeQL run on the private repository analyzed Python and JavaScript/TypeScript, then failed SARIF upload because GitHub code scanning is not enabled. | R6 release evidence is stronger locally and in CI for Semgrep/dependency-audit posture; CodeQL is workflow-proven but not upload/alert-proven until repository code scanning or a public-repo proof is available. |
| Credential handoff | `Docs/CREDENTIAL_HANDOFF.md` lists the exact GitHub App, OAuth, model, scanner, smoke-test, evidence, and stop-condition requirements for live proof. `scripts/credential_smoke.py` and `make credential-smoke` now roll GitHub OAuth, GitHub App, and model-provider smoke checks into a single redacted release artifact. | The next credentialed step is executable once secrets and a disposable demo repository are provided. |
...
| Webhook retention | `store_webhook_event` now persists minimized/redacted processing envelopes in `github_events.payload_json` with original payload SHA-256 metadata instead of durable full GitHub webhook bodies. Regression tests cover secret-bearing issue bodies, comment commands, and check-run summaries while preserving normalizer compatibility. `/repopilot` command receipt/reject/revise audit metadata now stores free-form args/reasons/instructions as presence, SHA-256, and length only. | Raw payload and command-argument retention risk is reduced locally; next privacy hardening should define an operator-facing retention policy for older rows/artifacts. |
| Draft PRs | `apps/api/app/services/draft_pr.py` creates local DB records by default and can call the real GitHub writer when write mode and credentials are present. PR summary, run trace, and security-detail contracts now expose `pr_mode`, `is_local_record`, and `github_url`, and the operator console labels local records while disabling GitHub-open actions until a real GitHub PR exists. | Local demo remains safe and clearer for first-run users; real write smoke test is pending credentials. |
| Evals | `apps/api/app/services/eval_runner.py` loads `packages/evals/benchmark_tasks.json`, validates 31 fixture tasks, delegates fixture repository/file/command checks to `repopilot_evals.FixtureVerifier`, records per-task outcomes, fixture repository pass rate, fixture file coverage, category pass rates, quality gates, optional observed plan-quality scores from `model_config.observed_plan_results`, optional observed patch-quality scores from `model_config.observed_task_results`, human edit distance when generated/reference diffs are supplied, and provider comparison results from `model_config.provider_eval_results`. `repopilot_evals.BenchmarkReportBuilder` and `make eval-report` now write Markdown/JSON local evidence under `Docs/eval-reports/`; `ProviderPlanningEvalRunner`, `make provider-planning-eval`, and `.github/workflows/provider-planning-eval.yml` can collect planning-only provider evidence from the encrypted local runtime secret store, an environment override, or a GitHub Actions secret using the shared OpenAI-compatible, Anthropic, or Gemini provider adapter layer. `ProviderRetrievalEvalRunner` and `make provider-retrieval-eval` can collect provider embedding retrieval evidence and feed retrieved file citations into the existing context-precision gate. `ProviderPatchEvalRunner`, `make provider-patch-eval`, and `.github/workflows/provider-patch-eval.yml` can collect provider patch-attempt evidence without mutating fixtures. `ProviderAppliedPatchEvalRunner`, `make provider-applied-patch-eval`, and `.github/workflows/provider-applied-patch-eval.yml` can apply provider-generated unified diffs to temporary fixture copies, run benchmark-declared validation commands, derive security outcomes from changed paths/secret-like diff content/high-risk workflow edits, and accept explicit no-patch block/escalation decisions for security fixtures. | Executable local fixture proof plus plan/context/patch-quality, human-edit-distance, provider-comparison scoring, a local baseline report, local/GitHub provider planning harnesses, local provider retrieval harnesses, local/GitHub provider patch-attempt harnesses, and applied-patch validation harnesses exist; credentialed provider-backed comparisons are pending. |
...
| Dashboard | `apps/web/app/operator-console.tsx` is a broad operator console with activity, settings, security lifecycle actions, runs, PRs, metrics, plan approval/rejection/revision, CI revision plans, and eval task outcomes. Recent copy cleanup labels patch validation, open findings, workflow-path findings, disabled explanation generation, unconfigured notifications, and refresh freshness without implying unavailable backend delivery or production proof. Release screenshots are now captured under `Docs/release-artifacts/` for dashboard desktop/mobile, plan review, agent runs, run trace, pull requests, security, evaluations, and settings; local plan-to-PR and governance visual-flow GIFs are generated with `make release-gifs`; the mobile shell now uses a horizontal top rail instead of clipping content behind a fixed sidebar. | Strong UI base with fewer truthfulness gaps, but live-provider/GitHub write states still depend on credentials; credentialed live-mode screenshots remain pending. |
...
| Local runtime platform | Docker Compose now runs API, web, worker, beat, Postgres, and Redis together. Web dev caches are isolated through named volumes for `node_modules` and `.next`; API/worker/beat share `agent_workspaces` for sandbox cleanup and `agent_artifacts` for artifact-backed evidence plus scheduled retention cleanup. `make start-local`/`make bootstrap` initializes local env values, starts the source-build stack, applies migrations, builds the sandbox image, and prints the local URLs; `make ghcr-start-local` now initializes local env values before pulling/starting released images; `make deployment-smoke` writes local runtime API/web smoke evidence under `Docs/release-artifacts/`. | First-run source-build and GHCR install friction is lower; actual GHCR publish/package visibility, credentialed GitHub/model proof, CodeQL upload proof, and production cloud/VM deployment proof are still pending. |
...
| Credential readiness snapshot | `make readiness-snapshot` | Passed: captured redacted running-app readiness under `Docs/release-artifacts/credential-readiness-snapshot.md`/`.json`, showing `model_mode=mock_model`, `github_mode=missing_credentials`, missing GitHub App/OAuth/model runtime secrets, local-record mode enabled, and GitHub writes disabled. |
| Credential smoke summary | `make credential-smoke` | Produces `Docs/release-artifacts/credential-smoke-summary.md`/`.json` with redacted GitHub OAuth, GitHub App, and model-provider smoke status. It exits successfully in `blocked` mode for local placeholder builds and becomes a pass/fail release gate after runtime secrets are saved. |
...
| 6. Validation, security, cleanup | Make generated patches trustworthy and operationally bounded. | Project detector, validation planner, command executor, secret redaction, Semgrep adapter, dependency audit adapter, CodeQL SARIF ingestion, GitHub CodeQL alert fetch hook, finding lifecycle, startup cleanup, scheduled cleanup. | Mostly complete locally. | Enable GitHub code scanning/Advanced Security or prove CodeQL on a public repo, archive successful CodeQL run/SARIF/alert evidence, add provider-backed secret-scan evidence, and run scanners in CI. | Failed validation or open high/critical findings block PR creation; stale workspaces are cleaned. |
...
| 5. Executor-mediated implementation | Mostly complete locally | Implementation agent proposes bounded workspace tool calls, executes through `ToolExecutor`, enforces approved write paths, filters secret-like files out of copied run workspaces and model-facing repo read/list/grep/tree tools, captures diff/hash, normalizes pytest validation commands, preserves the active local Python sandbox runtime, validates, and supports bounded retry. | Prove patch quality against executable fixture repositories and live model output; add batched read tools to reduce model/tool-call overhead. |
| 6. Validation, security, cleanup | Mostly complete locally | Project detector, validation planner, artifact-backed redacted validation logs, redacted sandbox stdout/stderr responses, evidence hashes, security finding lifecycle, startup cleanup service, scheduled Celery Beat cleanup over the shared `agent_workspaces` volume, command-backed Semgrep adapter, command-backed npm/pip dependency-audit adapter, CodeQL workflow file, guarded CodeQL SARIF ingestion, GitHub CodeQL alert fetch hook, fail-closed unavailable-tool findings, local scanner posture evidence through `make security-scanner-snapshot`, and CI scanner posture/version artifact upload through the `scanner-posture` job. | Prove CodeQL alert fetch with credentials, add provider-backed secret scanning, capture the new CI scanner artifact from GitHub Actions, and validate TypeScript/Python fixture repositories. |
...
| 9. Evals and quality gates | Partially complete | 31 fixture tasks exist with structured expected outcomes; Python and dashboard fixture repositories now exist and are verified for expected files, commands, and executable markers; eval runner records per-task outcomes, fixture repository pass rate, fixture file coverage, category pass rates, observed plan-quality results, context precision, observed patch-quality results, human edit distance, quality gates, shared CI/revision metrics, and DB-backed metrics; local provider planning, retrieval, patch-attempt, and applied-patch evals now read RepoPilot's encrypted runtime secret store first, with environment variables preserved as an override; `.github/workflows/provider-planning-eval.yml` can run planning-only provider tests from GitHub Actions with secret-name inputs and report artifacts; `.github/workflows/provider-retrieval-eval.yml` can run embedding-backed retrieval tests from GitHub Actions with secret-name inputs and report artifacts; `.github/workflows/provider-patch-eval.yml` can run provider patch-attempt tests and upload observed patch evidence; `.github/workflows/provider-applied-patch-eval.yml` can apply model-generated diffs to temporary fixture copies and upload validation evidence. | Run credentialed planning/retrieval/patch-attempt/applied-patch provider comparisons, connect CI-backed fixture execution, and publish measured thresholds. |
...
**Current status:** Implemented locally. `.env.example` and Compose expose the required placeholders, `make init-local-env` creates a git-ignored local `.env` with local-safe Compose values while leaving live GitHub/model credentials blank, GitHub App credentials can be saved through the encrypted runtime secret store, `/settings/github/app/verify` can verify installation-token creation, model verification now persists a provider/model verification marker, and `/settings/readiness` exposes `github_mode` and `model_mode`.
...
**Local evidence:** `apps/api/tests/test_init_local_env.py` verifies local env bootstrap generation, non-overwrite behavior, and preservation of extra existing local values; `apps/api/tests/test_release_targets.py` verifies the `init-local-env` Make target. API compile passed, full API suite previously passed with `180 passed`, web typecheck passed, web production build passed, fresh-database migration verification passed through `0006_llm_trace_metadata`, and runtime smoke checks passed with placeholder/local credentials.
...
**Current status:** Partially implemented locally and GitHub-ready for planning-only model tests. The model gateway can verify provider access and record live/fallback trace metadata. The eval package now includes `ProviderPlanningEvalRunner`, a planning-only OpenAI-compatible/native-provider harness that reads provider keys from the encrypted local runtime secret store, environment overrides, or the manual GitHub Actions workflow, prompts against fixture issue/file-inventory data, writes observed plan evidence, creates provider comparison summaries, and emits Markdown/JSON reports through `BenchmarkReportBuilder`. `ProviderRetrievalEvalRunner` and `make provider-retrieval-eval` now run embedding-backed fixture retrieval, write retrieved citations/scores, and feed those citations into the existing context-precision gate. This does not yet prove provider-backed patch generation or live CI behavior.
...
4. Compare live retrieval results against deterministic expectations through `make provider-retrieval-eval`.
...
**Current status:** Partially implemented locally and GitHub-ready for planning-only model tests. `packages/evals/fixtures/python-service` and `packages/evals/fixtures/web-dashboard` now provide executable fixture repositories. The eval runner verifies fixture paths, expected changed files, expected validation command targets, executable repository markers, fixture repository pass rate, and fixture file coverage. `packages/evals/repopilot_evals.PlanQualityScorer` grades optional observed plan evidence for target files, disallowed paths, expected validation commands, human-approval requirements, summary intent, and context citation precision. `packages/evals/repopilot_evals.PatchQualityScorer` grades optional observed patch evidence for changed files, disallowed paths, summary intent, validation commands, validation status, expected security result, and normalized human edit distance from generated/reference diffs or supplied distance values. `packages/evals/repopilot_evals.ProviderComparisonScorer` ranks optional provider/model evidence by blended plan quality, patch quality, context precision, human edit distance, cost, and latency. `packages/evals/repopilot_evals.BenchmarkReportBuilder` writes local Markdown/JSON evidence reports, and `Docs/eval-reports/v1-local-latest.md` now records the current baseline fixture report. `ProviderPlanningEvalRunner`, `ProviderRetrievalEvalRunner`, `ProviderPatchEvalRunner`, `ProviderAppliedPatchEvalRunner`, `make provider-planning-eval`, `make provider-retrieval-eval`, `make provider-patch-eval`, and `make provider-applied-patch-eval` can resolve provider credentials from RepoPilot's encrypted local runtime secret store before falling back to provider-specific environment variables; the manual GitHub Actions workflows still use repository secrets. These harnesses use the same provider request/response adapter layer as the API gateway for OpenAI-compatible providers, Anthropic Messages, Gemini GenerateContent, and OpenAI-compatible embeddings where applicable. Applied-patch evals copy fixture repositories to temporary workspaces, apply generated unified diffs, run only benchmark-declared validation commands, preserve the original fixtures, derive security outcomes from changed paths and diff content, and treat no-patch block/escalation provider decisions as valid security evidence only for matching security fixtures. The benchmark quality gates require observed plan-quality, context-precision, patch-quality, human-edit-distance, and provider-comparison coverage before v1 claims. Credentialed provider-backed comparisons remain pending.
...
**Current status:** Partially implemented locally. Semgrep runs when `SEMGREP_ENABLED=true`, dependency audit runs npm and pip-audit adapters when `DEPENDENCY_AUDIT_ENABLED=true`, disabled adapters return explicit skipped evidence, and enabled-but-unavailable tools produce high-severity persisted findings. Stale run workspaces are cleaned on API startup and through `repopilot.workspace.cleanup`, a Celery Beat scheduled task that skips non-terminal run IDs and removes old terminal/abandoned workspaces from the shared `agent_workspaces` volume. CodeQL support now includes `.github/workflows/codeql.yml`, `/security/codeql/recommendation` for a ready-to-use GitHub Actions workflow, `/security/runs/{run_id}/codeql/sarif` for guarded SARIF ingestion, and `/security/runs/{run_id}/codeql/alerts/fetch` for credential-gated GitHub code-scanning alert ingestion behind `CODEQL_ENABLED`. The GitHub workflow runs automatically on public repositories and on private repositories only when the repository variable `CODEQL_ENABLED=true` is set after GitHub code scanning/Advanced Security is available. `scripts/security_scanner_snapshot.py` and `make security-scanner-snapshot` now capture current scanner enablement, dependency manifests, CodeQL workflow presence, installed-tool availability, and explicit disabled/blocked reasons under `Docs/release-artifacts/`. The CI workflow now includes a `scanner-posture` job that installs Semgrep and pip-audit, runs the scanner snapshot with Semgrep/dependency-audit flags enabled, and uploads redacted Markdown/JSON version/posture evidence as a workflow artifact. Credentialed CodeQL alert evidence, provider-backed secret scanning, and downloaded CI scanner artifact evidence are still pending.
...
3. Fix text overflow, overlapping panels, unclear disabled states, and false "production-ready" labels. The dashboard mobile clipping issue is fixed, and the June 12 copy pass removed the strongest overclaims around production-ready code, automatic CI reruns, generated PR readiness, and provider privacy/source-transfer semantics. The repo now has a static UI truth/privacy guard in CI; continue this pass with rendered visual regression coverage and credentialed live-state screenshots.
...
**Current status:** Partially implemented locally. The deployment guide, demo script, release checklist, case study, release notes, core static screenshots, local visual-flow GIFs, scanner posture snapshot, credential smoke summary, and `scripts/deployment_validate.py` now exist in release-usable form and describe or verify the local stack, released-image GHCR path, single-VM path, managed Postgres/Redis option, secret handling, backups, rollback, local demo flow, credentialed GitHub smoke flow, Compose topology, env placeholders, release artifacts, and release evidence gates. `docker-compose.ghcr.yml` defines the prebuilt-image install path for API, worker, beat, web, Postgres, Redis, and sandbox image pulls without source bind mounts or dev commands. `make ghcr-start-local` initializes local env values, pulls images, starts the GHCR stack, and applies migrations for fresh-clone users; the individual `ghcr-config`, `ghcr-pull`, `ghcr-up`, and `ghcr-migrate` targets remain available. `make release-gifs` writes `Docs/release-artifacts/release-gifs.md`/`.json`, `make credential-smoke` writes `Docs/release-artifacts/credential-smoke-summary.md`/`.json`, `make security-scanner-snapshot` writes `Docs/release-artifacts/security-scanner-snapshot.md`/`.json`, `make deployment-validate` writes `Docs/release-artifacts/deployment-validation.md`, and `make deployment-smoke` writes `Docs/release-artifacts/deployment-runtime-smoke.md`/`.json`; both deployment checks currently pass for the local stack. `.github/workflows/release.yml` now supports manual dry runs, optional manual image publishing, and tag-triggered release evidence generation; it uploads deterministic local eval, source-boundary manifest, credential-smoke, scanner posture, and deployment-validation artifacts, then builds API/web/sandbox images with GHCR metadata, pushes them on tag releases or explicitly published manual runs, and uploads image digest evidence. Remaining work is proof beyond local validation: actual GHCR publish/package-visibility smoke, production-like cloud/VM deployment smoke, live GitHub smoke evidence, enabled CodeQL upload/alert proof, and provider-backed eval evidence are still pending.
...
| RP-9-02 | Eval runner | Partially implemented locally and GitHub-ready for planning-only model tests: fixture schema/repository checks run deterministically, optional observed plan-quality evidence can be supplied through `model_config.observed_plan_results`, optional observed patch-quality evidence can be supplied through `model_config.observed_task_results`, optional provider/model comparison evidence can be supplied through `model_config.provider_eval_results`, `BenchmarkReportBuilder`/`make eval-report` produce local Markdown/JSON evidence, and `ProviderPlanningEvalRunner`/`ProviderRetrievalEvalRunner`/`ProviderPatchEvalRunner`/`ProviderAppliedPatchEvalRunner`/`make provider-planning-eval`/`make provider-retrieval-eval`/`make provider-patch-eval`/`make provider-applied-patch-eval`/the manual provider eval workflows can collect provider evidence from RepoPilot's encrypted local runtime secret store first, then an environment override or GitHub Actions secret; applied-patch evals apply generated diffs to temporary fixture copies, run benchmark-declared validation commands, preserve original fixtures, and derive security outcomes from changed paths, secret-like diff content, high-risk workflow edits, or matching no-patch security decisions. | Local report produced from fixtures and observed plan/retrieval/patch-quality/provider evidence when provided; GitHub Actions artifact produced after a provider secret is configured. |
...
| RP-12-03 | Release workflows | API/web/sandbox image builds, tags, release notes, changelog. | Partially complete: release notes exist for the local v1.0 candidate, and the release workflow now has a manual/tag-triggered release-evidence job that uploads deterministic eval, source-boundary, credential-smoke, and deployment-validation artifacts before image builds; manual dry run `27059971285` passed with evidence upload plus API/web/sandbox image builds, and downloaded artifact hashes are archived under `Docs/release-artifacts/release-workflow-dry-run.*`. Final changelog remains pending. |
