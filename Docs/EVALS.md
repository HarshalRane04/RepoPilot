...
- `make provider-applied-patch-eval`: runs the provider applied-patch harness using the encrypted local runtime secret store first, then provider-specific environment variables as an override. It copies fixture repositories to temporary workspaces, applies model-generated unified diffs, runs only benchmark-declared validation commands, derives security outcomes from changed paths and diff content, accepts matching no-patch security decisions for block/escalation fixtures, and writes `Docs/eval-reports/v1-provider-applied-patch.*`.
