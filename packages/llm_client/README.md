...
The API is provider-agnostic today and records model names plus LLM trace metadata for planning. `/settings/readiness` now reports `MODEL_PROVIDER=mock` as a production blocker so local deterministic behavior is not mistaken for live LLM behavior. The API gateway supports OpenAI-compatible chat completions plus native Anthropic Messages and Gemini GenerateContent request/response shapes; future shared gateway utilities can move into this package once they no longer depend on API-specific trace persistence.
