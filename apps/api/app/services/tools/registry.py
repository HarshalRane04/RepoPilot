...
class ApplyPatchInput(WorkspacePathInput):
    diff: str = Field(
        ...
        description=(
            "Complete git-compatible unified diff containing diff --git, --- a/path, +++ b/path, "
            "and valid @@ hunk headers. Do not wrap the diff in Markdown fences."
        ),
    )
...
class WriteFileInput(WorkspacePathInput):
    path: str = Field(description="Repository-relative path already approved by the implementation plan.")
    content: str = Field(
        max_length=1_000_000,
        description="Complete replacement file content; use only when the entire file is known.",
    )
...
class ReplaceTextInput(WorkspacePathInput):
    ...
    old_text: str = Field(
        min_length=1,
        max_length=500_000,
        description="Exact existing text that occurs once in the target file.",
    )
    new_text: str = Field(
        max_length=500_000,
        description="Complete replacement for old_text, preserving surrounding content.",
    )
...
class ToolRegistry:
    ...
    def _register_defaults(self) -> None:
        self._register(name="repo.index", description="Index a local repository path into code context chunks.", input_model=RepoIndexInput, permission=ToolPermissionTier.DB_MUTATION, handler=_repo_index)
        ...
        self._register(name="plan.generate", description="Generate an implementation plan from issue and indexed context.", input_model=PlanGenerateInput, permission=ToolPermissionTier.DB_MUTATION, handler=_plan_generate)
        ...
        self._register(name="ci.analyze", description="Analyze CI input as non-promoting simulation evidence.", input_model=CiAnalyzeInput, permission=ToolPermissionTier.DB_MUTATION, handler=_ci_analyze)
        self._register(name="github.fetch_ci_logs", description="Fetch bounded, redacted GitHub check-run and annotation summaries for a pull request head.", input_model=GithubFetchCiLogsInput, permission=ToolPermissionTier.EXTERNAL_READ, handler=_github_fetch_ci_logs)
        ...
        self._register(name="eval.record_run", description="Record an evaluation report from current platform evidence.", input_model=EvalRecordInput, permission=ToolPermissionTier.DB_MUTATION, handler=_eval_record_run)
...
async def _workspace_apply_patch(db: AsyncSession, request: ToolCallRequest, payload: BaseModel) -> dict[str, Any]:
    ...
    actual_paths = {
        ...
        for change in diff_payload.get("changed_files", [])
        if isinstance(change, dict) and change.get("path")
    }
    try:
        if len(actual_paths) > args.max_changed_files:
            raise ToolBlocked("Applied patch changes more files than max_changed_files.")
    ...
    if not args.return_diff:
        return {"changed_paths": sorted(actual_paths)}
    return {"changed_paths": sorted(actual_paths), **diff_payload}
...
async def _sandbox_run_command(db: AsyncSession, request: ToolCallRequest, payload: BaseModel) -> dict[str, Any]:
    ...
    result = await SandboxRunner().run_command_async(
        SandboxCommandRequest(
...
async def _validation_run_tests(db: AsyncSession, request: ToolCallRequest, payload: BaseModel) -> dict[str, Any]:
    ...
    return await _run_validation_command(
        db,
        request=request,
        workspace_path=args.workspace_path,
        working_directory=args.working_directory,
...
async def _ci_analyze(db: AsyncSession, request: ToolCallRequest, payload: BaseModel) -> dict[str, Any]:
