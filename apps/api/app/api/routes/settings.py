...
class GitHubOAuthConfigRequest(BaseModel):
    github_client_id: str | None = Field(default=None, min_length=1, max_length=256)
    github_client_secret: str | None = Field(default=None, min_length=1, max_length=2048)
    github_owner_login: str | None = Field(
        default=None,
        min_length=1,
        max_length=39,
        pattern=r"^[A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?$",
    )
    session_secret_key: str | None = Field(default=None, min_length=32, max_length=4096)
    github_oauth_callback_url: str | None = Field(default=None, min_length=1, max_length=2048)
    web_app_url: str | None = Field(default=None, min_length=1, max_length=2048)
    github_api_base_url: str | None = Field(default=None, min_length=1, max_length=2048)
    github_web_base_url: str | None = Field(default=None, min_length=1, max_length=2048)
    ...
    @classmethod
    def validate_callback_path(cls, value: str | None) -> str | None:
        if value is None:
            return None
    ...
    @classmethod
    def validate_github_api_base_url(cls, value: str | None) -> str | None:
        return safe_github_api_base_url(value) if value is not None else None

    @field_validator("github_web_base_url")
    @classmethod
    def validate_github_web_base_url(cls, value: str | None) -> str | None:
        return safe_github_web_base_url(value) if value is not None else None
...
async def save_github_oauth_config(
    ...
    values = {
        ...
        "REPOPILOT_GITHUB_OWNER_LOGIN": request.github_owner_login,
    ...
    runtime_secret_store().save_values({key: value for key, value in values.items() if value is not None})
    return runtime_secret_store().summary(set(GITHUB_OAUTH_RUNTIME_SECRET_FIELDS))
...
async def save_model_provider_config(
    ...
    effective = effective_settings()
    selected_api_key = request.model_api_key or model_api_key_for_provider(request.provider)
    configured_key_providers = configured_model_api_key_providers()
    try:
    ...
    if request.model_api_key:
        ...
        values["MODEL_API_KEY_PROVIDER"] = request.provider
    elif configured_key_providers:
        # Pin legacy unscoped keys to the provider that owned them before changing MODEL_PROVIDER.
        values["MODEL_API_KEY_PROVIDER"] = configured_key_providers[0]
    store = runtime_secret_store()
...
async def verify_model_provider_config(
    ...
    if result.ok:
        ...
        runtime_secret_store().delete_values({"MODEL_PROVIDER_VERIFIED_AT", "MODEL_PROVIDER_VERIFIED_MODEL"})
...
async def runtime_policy() -> dict[str, object]:
    ...
    return {
        ...
        "max_cost_per_run": effective.max_cost_per_run,
        "cost_currency": "USD",
    }
...
async def _model_provider_status() -> dict[str, object]:
    ...
    return {
        ...
        "configured_api_key_providers": list(configured_key_providers),
        "base_url": effective.model_base_url or (provider.default_base_url if provider else None),
